            ...
            <div class="card-body">
                <form  action="/profilelists">
                    ...
                    <div class="form-group col-6">
                        ...
                        value="{{old('name')}}" placeholder="نام را بنوسید">
                    </div>
                    <div class="form-group col-6">
                        <label class="col-form-label col-lg-3">تخلص:<span class="text-danger">*</span></label>
                        <input type="text" class="form-control" name="lastname" 
                        value="{{old('name')}}" placeholder="تخلص را بنوسید">
                    </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-6">
                            <label class="col-form-label col-lg-3">Firstname:<span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="firstnameEn" 
                            value="{{old('name')}}" placeholder="Enter the first name">
                        </div>
                        <div class="form-group col-6">
                            ...
                            value="{{old('name')}}" placeholder="Enter the last name">
                        </div>
                        </div>
        ...
        <div class="col-md-12">
            ...
            <div class="datatable-scroll">
                <table class="table datatable-basic dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                    <thead>
                        <tr role="row" style="background-color: midnightblue; color:white">
                        ...
                        @foreach ($candidateDetails as $can) {
                            ...
                            <tr>
                                <td>{{$can->firstname}}</td>
                                <td>{{$can->lastname}}</td>
                                <td>{{$can->fathername}}</td>
                                <td>{{$can->grandfathername}}</td>
                                <td>{{$can->dateofbirth}}</td>
                                <td>{{$can->genderID == 1 ?'مرد':'زن'}}</td>
                                <td>{{$can->martialstatusID == 1 ?'مجرد':'متحل'}}</td>
                                    ...
                                    <div class="list-icons">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <a  href="/updatepro/{{$can->id}}" class="dropdown-item">
                                                    <i class="icon-unlocked2"></i>بروز رسانی</a>
                                                <a href="/createjob/{{$can->id}}" class="dropdown-item"data-toggle="modal" data-target="#modal_default{{$can->id}}" ><i class="icon-pencil"></i>وظیفه کاندید</a>
                                                <a href="/accesstouser/{{$can->id}}" class="dropdown-item"><i class="icon-accessibility "></i>آدرس کاندید</a>
                                                <a href="/accesstouser/{{$can->id}}" class="dropdown-item"><i class="icon-accessibility "></i>اقارب کاندید</a>
                                            </div>
                                        </div>
                                    </div>
                                </td>
